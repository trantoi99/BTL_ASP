...
function getListLoaiSach() {
    postData('GET', '/LoaiSach/ListLoaiSach', null).then(function (data) {
        if (data != null) {
            var html = '';
...
function SaveChange() {
    ...
    formData.append('Maloai', $("#txtMaLoai").val());
    formData.append('Tenloai', $("#txtTenLoai").val());
    postData('POST', '/LoaiSach/SaveChange', formData).then(function (data) {
        if (data.Error == true) {
...
function Delete(Maloai) {
    ...
    postData('DELETE', '/LoaiSach/Delete', formData).then(function (msg) {
        if (msg.Error == true) {
...
function resetinput() {
    ...
    $("#txtMaLoai").val("");
    $("#txtTenLoai").val("");
    document.getElementById('txtMaLoai').disabled = false;
}

function getId(Maloai) {
    let formData = new FormData();
    formData.append('MaLoai', Maloai);
    postData('POST', '/LoaiSach/getInfoId', formData).then(function (data) {
        if (data != null) {
            $("#txtId").val("1");
            $("#txtMaLoai").val(data.Maloai);
            document.getElementById('txtMaLoai').disabled = true;
            $("#txtTenLoai").val(data.Tenloai);
        }
        else {
