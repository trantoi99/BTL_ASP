...
function getlistSinhvien() {
    postData('GET', '/Sinhvien/listSinhvien', null).then(function (data) {
        //debugger
        if (data != null) {
            var html = '';
...
function SaveChange() {
    ...
    formData.append('Masinhvien', $("#txtMaSinhVien").val());
    formData.append('Tensinhvien', $("#txtTenSinhVien").val());
    formData.append('Lop', $("#txtLop").val());

    postData('POST', '/Sinhvien/SaveChange', formData).then(function (data) {
        if (data.Error == true) {
...
function Delete(Masinhvien) {
    ...
    postData('DELETE', '/SinhVien/Delete', formData).then(function (msg) {
        if (msg.Error == true) {
...
function resetinput() {
    ...
    document.getElementById('txtMaSinhVien').disabled = false;
}

function getId(col) {
    //debugger
    let formData = new FormData();
    formData.append('Masinhvien', col);
    postData('POST', '/Sinhvien/getInfoId', formData).then(function (data) {
        if (data != null) {
            $("#txtId").val("1");
            $("#txtMaSinhVien").val(data.Masinhvien);
            document.getElementById('txtMaSinhVien').disabled = true;
            $("#txtTenSinhVien").val(data.Tensinhvien);
            $("#txtLop").val(data.Lop);
