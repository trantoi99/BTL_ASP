...
function getListSach() {
    postData('GET', '/Sach/ListSach', null).then(function (data) {
        if (data != null) {
            var html = '';
...
function SaveChange() {
    ...
    formData.append("Masach", $("#txtMaSach").val());
    formData.append("Tensach", $("#txtTenSach").val());
    formData.append("Namxuatban", $("#txtNamXuatBan").val());
    formData.append("Soluong", $("#txtSoLuong").val());
    formData.append("ngonngu", $("#txtngonngu").val());
    formData.append("nhaxuatban", $("#txtnhaxuatban").val());
    formData.append("tacgia", $("#txttacgia").val());
    formData.append("loai", $("#txtloai").val());
    formData.append("vitri", $("#txtvitri").val());
    formData.append("kho", $("#txtkho").val());
   // debugger
    postData('POST', '/Sach/SaveChange', formData).then(function (data) {
        if (data.Error == true) {
...
function Delete(Masach) {
    let formData = new FormData();
    formData.append('Masach', Masach);
    postData('DELETE', '/Sach/Delete', formData).then(function (msg) {
        if (msg.Error == true) {
...
function getId(Masach) {
    let formData = new FormData();
    formData.append('Masach', Masach);
    postData('POST', '/Sach/getInfoId', formData).then(function (data) {
        
        if (data != null) {
            $("#txtId").val("1");
            //$("#txtTenSach").val(data.Tensach);
            $("#txtNamXuatBan").val(data.Namxuatban);
            $("#txtSoLuong").val(data.Soluong);
            $("#txtngonngu").val(data.Mangonngu);
            $("#txtnhaxuatban").val(data.Manhaxuatban);
            $("#txttacgia").val(data.Matacgia);
            $("#txtloai").val(data.Makho);
            $("#txtvitri").val(data.Mavitri);
            $("#txtkho").val(data.Makho);
            $("#txtMaSach").val(data.Masach);
            document.getElementById('txtMaSach').disabled = true;
            $("#txtTenSach").val(data.Tensach);
        }
        else {
